...
import { accounts, debts, entries, goals, subscriptions } from '../db/schema.js';
...
stateRouter.get('/', async (req, res) => {
  ...
  const [userAccounts, userGoals, userSubscriptions, userDebts, userEntries] = await Promise.all([
